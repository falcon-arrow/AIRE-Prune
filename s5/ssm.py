...
class S5SSM(nn.Module):
    ...
    def setup(self):
        ...
        if self.pruning:
            if self.bidirectional:
                Score = (np.linalg.norm(self.C_tilde[:,:self.P], axis=0)+np.linalg.norm(self.C_tilde[:,-self.P:], axis=0))/2 **2 * np.linalg.norm(self.B_bar, axis=1)**2  / ((1-(np.abs(self.Lambda_bar))**2) + 1e-8)
                # Score = (0.5 * ((np.linalg.norm(self.C_tilde[:, :self.P],  axis=0)**2) + (np.linalg.norm(self.C_tilde[:, -self.P:], axis=0)**2))) * (np.linalg.norm(self.B_bar, axis=1)**2)  / ((1-(np.abs(self.Lambda_bar))**2) + 1e-8)
                # Score = (np.linalg.norm(self.C_tilde[:,:self.P], axis=0)+np.linalg.norm(self.C_tilde[:,-self.P:], axis=0))/2 **2 * np.linalg.norm(self.B_bar, axis=1)**2 / ((1-np.abs(self.Lambda_bar))**2 + 1e-8)

            else:
                Score = (np.linalg.norm(self.C_tilde, axis=0)**2) * (np.linalg.norm(self.B_bar, axis=1)**2) / ((1-(np.abs(self.Lambda_bar))**2) + 1e-8)
                # Score = np.linalg.norm(self.C_tilde, axis=0)**2 * np.linalg.norm(self.B_bar, axis=1)**2 / ((1-np.abs(self.Lambda_bar))**2 + 1e-8)

            order = np.argsort(Score)
            ENERGYscore = np.sort(Score) / np.cumsum(np.sort(Score)[::-1])[::-1]
            self.ENERGYscore = Score.at[order].set(ENERGYscore)
    ...
    def __call__(self, input_sequence, global_th):
        ...
        Args:
             ...
             global_th (float32): global threshold for AIRE pruning (P, )
        Returns:
            output sequence (float32): (L, H)
        """

        if self.pruning:
            mask = self.ENERGYscore >= global_th
