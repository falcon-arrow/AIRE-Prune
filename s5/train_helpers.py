...
def create_train_state(model_cls,
    ...
    if opt_config in ["standard"]:
        ...
        if dt_global:
            ...
            ssm_fn = map_nested_fn(
                ...
                else ("none" if k in ["ENERGYscore"] else "regular")
            )
        ...
        if dt_global:
            ...
            ssm_fn = map_nested_fn(
                ...
                else ("none" if k in ["ENERGYscore"] else ("ssm_decay" if k in ["B"] else "regular"))
            )
        tx = optax.multi_transform(
            {
                "none": optax.inject_hyperparams(optax.adamw)(learning_rate=ssm_lr,
                                                              weight_decay=weight_decay),
                "ssm": optax.inject_hyperparams(optax.adam)(learning_rate=ssm_lr),
                "ssm_decay": optax.inject_hyperparams(optax.adamw)(
                    learning_rate=ssm_lr, weight_decay=weight_decay  # or ssm_wd
                ),
                "regular": optax.inject_hyperparams(optax.adamw)(learning_rate=lr,
                                                                 weight_decay=weight_decay),
            },
    ...
    param_sizes = map_nested_fn(lambda k, param: 0 if k == "ENERGYscore" else param.size * (2 if fn_is_complex(param) else 1))(params)
...
def validate(state, model, testloader, seq_len, in_dim, batchnorm, step_rescale=1.0, global_th=0):
    ...
    for batch_idx, batch in enumerate(tqdm(testloader)):
        ...
        loss, acc, pred, ENERGYscores = eval_step(inputs, labels, integration_timesteps, global_ths, state, model, batchnorm)
        losses = np.append(losses, loss)
        accuracies = np.append(accuracies, acc)

    aveloss, aveaccu = np.mean(losses), np.mean(accuracies)
    return aveloss, aveaccu, ENERGYscores
...
def eval_step(batch_inputs,
    ...
    if batchnorm:
        logits, ENERGYscores = model.apply({"params": state.params, "batch_stats": state.batch_stats},
                             batch_inputs, batch_integration_timesteps, batch_global_th,
                             )
    else:
        logits, ENERGYscores = model.apply({"params": state.params},
                             batch_inputs, batch_integration_timesteps, batch_global_th,
                             )
    ...
    return losses, accs, logits, ENERGYscores
