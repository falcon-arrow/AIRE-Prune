...
class StackedEncoderModel(nn.Module):
    ...
    def __call__(self, x, integration_timesteps, global_ths):
        ...
        for layer in self.layers:
            x, ENERGYscore = layer(x, global_ths)
            ENERGYscores.append(ENERGYscore)
        return x, np.concatenate(ENERGYscores)
...
batch_masked_meanpool = jax.vmap(masked_meanpool, in_axes=(0, 0), out_axes=0)
...
class ClassificationModel(nn.Module):
    ...
    def __call__(self, x, integration_timesteps, global_ths):
        ...
        x, ENERGYscores = self.encoder(x, integration_timesteps, global_ths)
        ...
        x = self.decoder(x)
        return nn.log_softmax(x, axis=-1), ENERGYscores
...
class RetrievalModel(nn.Module):
    ...
    def __call__(self, input, integration_timesteps, global_ths):  # input is a tuple of x and lengths
        ...
        return nn.log_softmax(out, axis=-1), ENERGYscores
